...
template <class Type>
void QbitRegister<Type>::applySwap(unsigned qubit1, unsigned qubit2)
{
...
template <class Type>
void QbitRegister<Type>::applySqrtISwap(unsigned qubit1, unsigned qubit2)
{
...
template <class Type>
void QbitRegister<Type>::applyISwapRotation(unsigned qubit1, unsigned qubit2, TM2x2<Type> const& m)
{
...
template <class Type>
void QbitRegister<Type>::applyISwap(unsigned qubit1, unsigned qubit2)
{
  openqu::TinyMatrix<Type, 2, 2, 32> g;
  g(0, 0) = g(1, 1) = {0, 0};
  g(0, 1) = g(1, 0) = {0, 1};
...
template <class Type>
void QbitRegister<Type>::apply4thRootISwap( unsigned qubit1, unsigned qubit2)
{
...
template <class Type>
bool QbitRegister<Type>::applySwap_helper(unsigned qubit1_, unsigned qubit2_, TM2x2<Type> const&m)
{
#if 0
  ...
  if (qubit1 < M && qubit2 < M) {
    ...
    {  // this is itask
       ...
       MPI_Sendrecv(&(state[start_ind]), lcl_size_half, MPI_DOUBLE_COMPLEX,
                    jtask, tag1,
                    &(tmpstate[0]), lcl_size_half, MPI_DOUBLE_COMPLEX,
                    jtask, tag2,
                    comm, &status);
       #else
       MPI_Sendrecv_x(&(state[start_ind]), lcl_size_half, 
                      jtask, tag1,
                      &(tmpstate[0]), lcl_size_half, 
                      jtask, tag2,
                      comm, &status);
       #endif
       ...
       {
         #if 0
         MPI_Sendrecv(&(tmpstate[0]), lcl_size_half, MPI_DOUBLE_COMPLEX,
                      jtask, tag1,
                      &(state[0]), lcl_size_half, MPI_DOUBLE_COMPLEX,
                      jtask, tag2,
                      comm, &status);
         #else
         MPI_Sendrecv_x(&(tmpstate[0]), lcl_size_half,
                        jtask, tag1,
    ...
    {  // this is jtask
       ...
       state0 = (qubit1 < M - 1) ? (tmpstate + delta1) : tmpstate;
       state1 = state;
       for (std::size_t j = 0; j < lcl_size_half; j += 2 * delta1)
         for(std::size_t i = j; i < std::min(lcl_size_half, j + delta1); i++)
         {
...
template <class Type>
void QbitRegister<Type>::swap(unsigned b1, unsigned b2)
{
  assert(b1 < nqbits);
  assert(b2 < nqbits);
